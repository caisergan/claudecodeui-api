...
const ALL_PROVIDERS = providerRegistry.listProviders().map(p => p.id);
...
async function computeProviderUsageLimit(provider) {
  ...
  let status;
  try {
    const resolved = providerRegistry.resolveProvider(provider);
    status = await resolved.auth.getStatus();
  } catch {
    status = {
      installed: false,
      authenticated: false,
